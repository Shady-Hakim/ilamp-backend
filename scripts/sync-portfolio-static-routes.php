...
$publishedSlugs = PortfolioProject::query()
    ...
    if (is_array($manifestFiles)) {
        $exportedSlugs = collect($manifestFiles)
            ...
            ->map(function (string $path): ?string {
                if (! preg_match('#^portfolio/([^/]+)/index\.html$#', $path, $matches)) {
                    return null;
                }
...
collect(File::directories(public_path('portfolio')))
...
PortfolioProject::query()
    ->where('is_published', true)
    ->get()
    ->each(fn (PortfolioProject $project) => app(FrontendStaticRouteService::class)->syncPortfolioProject($project));

fwrite(STDOUT, "Synced published portfolio static routes.\n");
